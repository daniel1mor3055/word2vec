...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs -- the gradient with respect to all the outside word vectors
                    ...
                    (dJ / dU)
    """
    u0 = outsideVectors[outsideWordIdx]
    vc = centerWordVec
    y_hat = np.exp(np.matmul(outsideVectors, vc))

    loss = -np.log(np.exp(np.dot(u0, vc)) / np.sum(y_hat))

    vocab_size = outsideVectors.shape[0]
    y_hat /= np.sum(y_hat)

    y = np.eye(vocab_size)[outsideWordIdx]

    gradCenterVec = np.matmul((y_hat - y), outsideVectors)

    gradOutsideVecs = np.outer(y_hat, vc)
    gradOutsideVecs[outsideWordIdx] = (y_hat[outsideWordIdx] - 1) * vc
...
def negSamplingLossAndGradient(
    ...
    vc = centerWordVec
    u0vc = np.dot(u0, vc)
    sig_u0vc = sigmoid(u0vc)


    loss = -np.log(sig_u0vc) - np.sum(
        np.log(sigmoid(-np.matmul(outsideVectors[negSampleWordIndices], vc))))

    gradCenterVec = -(1-sig_u0vc) * u0 + np.matmul(
        np.transpose(outsideVectors[negSampleWordIndices]),
        sigmoid(-np.matmul(outsideVectors[negSampleWordIndices], vc)) * np.exp(
            np.matmul(outsideVectors[negSampleWordIndices], vc)))

    gradOutsideVecs = np.zeros(outsideVectors.shape)

    dj_du0 = -(1-sig_u0vc) * vc
    gradOutsideVecs[outsideWordIdx] = dj_du0

    for ind in indices[1:]:
        u_ws = outsideVectors[ind]
        sig_u_ws_vc = sigmoid(-np.dot(u_ws, vc))
        gradOutsideVecs[ind] += (1 - sig_u_ws_vc) * vc
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        loss, centerGrad, outsideGrads = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]],
                                                                 word2Ind[word], outsideVectors, dataset)
        gradCenterVecs[word2Ind[currentCenterWord]] += centerGrad
        gradOutsideVectors += outsideGrads
        total_loss += loss

    return total_loss, gradCenterVecs, gradOutsideVectors
